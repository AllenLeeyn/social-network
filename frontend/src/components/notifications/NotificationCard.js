  ...
  const handleNotificationRespond = async (status) => {
    try {
      if (notification.target_detailed_type === "follow_request") {
        await submitFollowResponse({
          follower_uuid: notification.target_uuid,
          status,
        });
      } else if (notification.target_detailed_type === "group_invite") {
        await submitGroupRequestOrInviteResponse({
          follower_uuid: notification.to_user_uuid,
          ...
          <div className="notification-actions">
            {acceptRejectActions.includes(notification.target_detailed_type) &&
              (!notification.is_read ? (
                <div>
                  <button onClick={() => handleNotificationRespond("accepted")}>
                    Accept
                  </button>
                  <button onClick={() => handleNotificationRespond("declined")}>
                    Decline
                  </button>
                </div>
              ) : (
                  ...
                  onClick={(e) => {
                    ...
                    const href =
                      notification.target_detailed_type === "follow_request"
                        ...
                        : notification.target_detailed_type ===
                            "group_invite" ||
